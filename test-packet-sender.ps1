# Script para testar o envio de pacotes UDP para a porta 5050
# Este script envia pacotes simulados para testar o AlbionOnlineSniffer

param(
    [int]$Count = 5,
    [int]$Delay = 1000,
    [string]$Message = "Teste via PowerShell"
)

Write-Host "=== Teste de Pacotes UDP - Porta 5050 ===" -ForegroundColor Green
...
switch ($choice) {
    ...
    "3" {
        $count = Read-Host "Quantos pacotes enviar? (padrão: $Count)"
        if ([string]::IsNullOrEmpty($count)) { $count = $Count }
        else { $count = [int]$count }
        
        $delay = Read-Host "Delay entre pacotes em ms? (padrão: $Delay)"
    ...
    default {
        ...
        Send-UdpPacket -Message $Message
    }
}

Write-Host ""
Write-Host "Teste concluído!" -ForegroundColor Green
Write-Host "Verifique os logs do AlbionOnlineSniffer para ver se os pacotes foram recebidos." -ForegroundColor Cyan 